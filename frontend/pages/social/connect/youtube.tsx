/**
 * YouTube OAuth connect - redirects to backend OAuth flow
 */
...
export default function ConnectYouTube() {
    ...
    apiFetch('/api/social/connect', {
      ...
      .then((data) => {
        if (data.success && data.url) {
          ...
          router.replace(`/social?error=${encodeURIComponent(data.error ?? 'Failed to connect YouTube')}`)
        }
      })
      .catch((e) => {
  ...
  return (
    <>
      <Head><title>Connect YouTube – Cutnary</title></Head>
      <div className="flex min-h-screen items-center justify-center bg-[#0a0a0b]">
        <p className="text-zinc-400">Redirecting to YouTube…</p>
      </div>
    </>
  )
}
